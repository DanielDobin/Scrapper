import { toBase64 } from "../utils/conversions.js";
...
export class Solver {
    ...
    async poll(captcha) {
        try {
            ...
            switch (response.status) {
                case "ready":
                    return captcha.resolve(response);
                    break;
                case "processing":
                    captcha.polls++;
                default:
     ...
     */
    async registerPollEntry(task, delay = 5000) {
     ...
     * @throws {SolverError}
     */
    async imageCaptcha(image, extras = {}) {
     ...
     * })
     */
    async textCaptcha(comment, language = "en") {
        const cid = await this.newTask({
            task: {
     ...
     */
    async rotateCaptcha(image, extras = {}) {
        const cid = await this.newTask({
            task: {
                type: "RotateTask",
                body: toBase64(image),
     ...
     */
    async CoordinatesCaptcha(image, extras = {}) {
        const cid = await this.newTask({
            task: {
     ...
     */
    async gridCaptcha(image, extras) {
        const cid = await this.newTask({
            task: {
                type: "GridTask",
                body: toBase64(image),
     ...
     */
    async boundingBox(image, extras) {
        const cid = await this.newTask({
            task: {
     ...
     */
    async recaptchaV2(sitekey, pageurl, proxied = false, enterprise = false, extras) {
        const cid = await this.newTask({
            task: {
     ...
     * @returns {Promise<CaptchaResult<RecaptchaResult>>}
     */
    async recaptchaV3(sitekey, pageurl, extras) {
        const cid = await this.newTask({
            task: {
     ...
     */
    async hcaptcha(sitekey, pageurl, proxied = false, extras) {
        const cid = await this.newTask({
            task: {
     ...
     */
    async geetest(pageurl, gt, challenge, proxied = false, extra) {
        const cid = await this.newTask({
            task: {
     ...
     */
    async turnstile(pageurl, sitekey, proxied = false, extra) {
        const cid = await this.newTask({
            task: {
     ...
     * @returns {Promise<CaptchaResult<TurnstileResult>>}
     */
    async cloudflareTurnstile(pageurl, sitekey, proxied = false, extras) {
        const cid = await this.newTask({
            task: {
                type: proxied ? "TurnstileTask" : "TurnstileTaskProxyless",
                websiteURL: pageurl,
     ...
     */
    async capypuzzle(pageurl, sitekey, proxied = false, extras) {
        const cid = await this.newTask({
            task: {
     ...
     */
    async keycaptcha(proxied = false, extras) {
        const cid = await this.newTask({
            task: {
     ...
     */
    async leminPuzzle(siteurl, captchaId, divId, proxied = false, extras) {
        const cid = await this.newTask({
            task: {
                websiteURL: siteurl,
                captchaId: captchaId,
     ...
     */
    async amazonCaptcha(siteurl, sitekey, iv, context, proxied = false, extras) {
        const cid = await this.newTask({
            task: {
     ...
     */
    async cyberSiARA(siteurl, masterurlid, userAgent, proxied = false, extras) {
        const cid = await this.newTask({
            task: {
     ...
     * @returns {Promise<CaptchaResult<SingleTokenResult>>}
     */
    async mtCaptcha(siteurl, sitekey, proxed = false, extras) {
        const cid = await this.newTask({
            task: {
     ...
     * @returns {Promise<CaptchaResult<SingleTokenResult>>}
     */
    async cutCaptcha(siteurl, miserykey, proxied = false, extras) {
        const cid = await this.newTask({
            task: {
     ...
     */
    async datadome(siteurl, captchaurl, extras) {
        const cid = await this.newTask({
            task: {
                type: "DataDomeTask",
                websiteURL: siteurl,
     ...
     * @returns {Promise<CaptchaResult<SingleTokenResult>>}
     */
    async friendlyCaptcha(siteurl, sitekey, proxied = false, extras = {}) {
        const cid = await this.newTask({
            task: {
